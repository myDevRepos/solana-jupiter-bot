...
const setup = async () => {
	...
	try {
		...
		try {
			...
			// 	)}\n`
			// );
			console.log(`\n	Loading tokens failed!\n	Please run the Wizard to generate it using
					yarn start`);
			throw error;
		}
		...
		try {
			...
			console.log(
				"Wallet check failed! \n	Please make sure that  SOLANA_WALLET_PRIVATE_KEY inside .env file is correct"
			);
			// spinner.text = chalk.black.bgRedBright(
...
const getInitialotherAmountThreshold = async (
	...
	try {
		...
		if (routes?.routesInfos?.length > 0) console.log("Routes computed!");
		else
			console.log(
		...
		console.log("Computing routes failed!\n");
